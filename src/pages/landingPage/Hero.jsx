...
const Hero = ({ isHeroVisible = false }) => {
  ...
  return (
    ...
    >
      ...
      <SvgIcon
        icon={disk}
        className={`transition duration-200 delay-200  absolute max-lg:top-[70%] size-[88px] top-1/2 left-20 animate-spin`}
      />
      <ImgIcon
        icon="bi bi-mic"
      ...
      <ImgIcon
        icon="bi bi-boombox"
        className={`absolute top-[20%] max-lg:top-[14%] max-md:top-[12%] max-md:left-[40%]  -translate-x-1/2  ${
          isHeroVisible ? " animate-bounce left-[47%]" : "left-1/2"
        }`}
      />
      <div className="w-[128px] h-10 max-md:top-[25%]  border-t-4 border-black absolute  left-1/2 top-[35%] max-lg:top-[28%] -translate-x-1/2"></div>
      ...
      <div
        className={` -translate-x-1/2 top-[65%] left-1/2 transition duration-200 ease-out flex items-center justify-evenly w-full flex-row max-md:flex-col max-md:gap-y-10 ${
          isHeroVisible
            ? " translate-y-[20rem] scale-[0.6] opacity-0 backdrop-blur-none"
