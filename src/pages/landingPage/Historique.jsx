...
const Historique = () => {
  ...
  return (
    <section className="space-y-10 mt-[172px] box  y" id="Historique">
      <p className="text-black text-title-2 max-md:text-subtitle-2 max-xl:text-center">Historique</p>
      <div className="flex items-center justify-between max-xl:flex-col max-xl:gap-y-10">
        <div className="bg-primary-10 text-black-60 rounded-lg p-6 flex gap-6 flex-col items-left max-w-[648px] card-anim">
          <p className="text-base ">
            { showMore ? ABOUT_TEXT : ABOUT_TEXT.slice(0,maxTextLen) + ' ...' }
          </p>
          <div className="">
            <ButtonLg text={`${showMore ? 'Reduire' : "Afficher plus"}`} color="bg-black-10" handleClick={() => setShowMore((prev) => !prev)} />
          </div>
        </div>
